...
class Fuses():
    ...
    def __init__(self,
        ...
        self.fdata = '-U lfuse:w:{}:m -U hfuse:w:{}:m -U efuse:w:{}:m'.format(
            hex(self.Low), hex(self.High),
            hex(self.Ext))

    def read(self, mcu="m644p"):
        process = Popen([
            'avrdude', '-c', 'usbasp', '-p',
            mcu, '-v', '-u', '-U',
            'lfuse:r:-:h', '-U', 'hfuse:r:-:h', '-U',
            'efuse:r:-:h'
        ],
                        stdout=PIPE)
    ...
    def write(self, mcu="m644p"):
        print(self.fdata, '\n' * 10)
        process = Popen([
            'avrdude', '-c', 'usbasp',
            '-p', mcu, '-v', '-e',
            '-u', '-U', self.fdata], stderr=PIPE)
    ...
    def verbose(self, low, high, ext):
        ...
        CKSEL1 = (low >> 1) & 1
        CKSEL0 = low & 1
        print('CKDIV8={}, CKOUT={}, SUT1={}, SUT0={}, CKSEL3={}, CKSEL2={}, CKSEL1={}, CKSEL0={}'.format(
        ...
        BOOTSZ0 = (high >> 1) & 1
        BOOTRST = high & 1
        print('OCDEN={}, JTAGEN={}, SPIEN={}, WDTON={}, EESAVE={}, BOOTSZ1={}, BOOTSZ0={}, BOOTRST={}'.format(
...
def read(format='hex', mcu="m644p"):
    ...
    s = f'avrdude -c usbasp -p {mcu} -v -U flash:r:flash.{format}:{f}'
...
def setfuses(low='0xEF', high='0xD8', ext='0xFF', mcu="m644p"):
    process = Popen([
        'avrdude', '-c', 'usbasp', '-p', mcu,
        '-v', '-e', '-u', '-U', 'efuse:w:{}:m'.format(ext), '-U',
        'hfuse:w:{}:m'.format(high), '-U', 'lfuse:w:{}:m'.format(low)
    ], stderr=PIPE)
    out, err = process.communicate()
    exit_code = process.wait()
...
def boot(mcu="m644p"):
    process = Popen(['avrdude', '-c', 'usbasp', '-p', mcu, '-v', '-U', 'flash:w:GprsBootloader.hex'], stderr=PIPE)
    out, err = process.communicate()
    exit_code = process.wait()
    return err
...
def flash(mcu="m644p"):
    process = Popen(['avrdude', '-c', 'usbasp', '-p', mcu, '-v', '-D', '-U', 'flash:w:BankSecurity.hex'],
                    stderr=PIPE)
    out, err = process.communicate()
    exit_code = process.wait()
    return err
